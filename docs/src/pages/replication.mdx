...
Preupdate hook fires BEFORE each row modification
  ↓
Extract: table, rowKey, oldValues, newValues
  ↓
...
Value (IntentEntry):
{
  ...
  Operation: 2,                    // UPDATE
  Table:     "users",
  RowKey:    "users:1",            // Single PK format
  OldValues: {
...
```
RowKey Format: "order_items:c:MTAw:NDI="
...
```
WriteIntent(table, rowKey)
  ↓
hash = XXH64(table + ":" + rowKey)
  ↓
filter.Check(hash)
  │
...
```go
func intentLockFor(table, rowKey string) *sync.Mutex {
    return &intentLocks[xxhash.Sum64String(table+":"+rowKey) % 256]
}
```
...
| `/intent/{table}/{rowKey}` | Write intents |
| `/intent_txn/{txnID}/{table}/{rowKey}` | Intent index by transaction |
...
│  CDC Entry created:                                                         │
│    Key: /cdc/0x0001A2B3C4D50001/1                                           │
│    Value: {Op: UPDATE, Table: "users", RowKey: "users:1",                   │
