...
If you plan to use bulk ingest with `LOAD DATA LOCAL INFILE`, enable local infile in your client:

```bash
mysql --local-infile=1 -h localhost -P 3306 -u root
```
...
- Use [Vector Search](/vector-search) if you want to store embeddings and run RAG-style semantic search inside Marmot.
- Use [SQL Reference](/sql-reference) to check supported MySQL syntax and Marmot-specific statements.
- Use [Operations](/operations) before running multi-node clusters outside local development.
