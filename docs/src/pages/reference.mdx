...
- Vector index DML follows ordinary row-level CDC. Vector DDL replicates compact control metadata; segment files, rowmaps, centroids, PQ codebooks, and overlay journals are local derived state.
- SQL tuning is through `CREATE VECTOR INDEX ... WITH (nlist = ..., nprobe = ..., max_norm = ...)` and per-connection `@@marmot_vec_*` session variables. See [Vector Search](/vector-search) for the startup guide and parameter details.
