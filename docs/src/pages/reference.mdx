...
- `data_dir`: Reserved for future vector-engine storage. Leave unset for current releases.
- `cache_bytes`: Caps the legacy per-index Go partition cache. The packed mmap snapshot and always-resident delta buffer remain available even when this is `0`.

**Runtime note:**
- The default read path is the packed streaming executor with resident delta.
- Set `cache_bytes` to a non-zero value only if you explicitly want to budget memory for the older Go partition cache.
