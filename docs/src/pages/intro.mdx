...
In Marmot every row is uniquely mapped to a JetStream in NATS. This guarantees that for a node to publish changes about a row it has to go through 
same JetStream as everyone else. If two nodes both perform a change to same row, both of the nodes will compete to publish their change changes to
JetStream cluster, and due to [RAFT quorum](https://docs.nats.io/running-a-nats-service/configuration/clustering/jetstream_clustering#raft) 
requirements one one of the writer will be able to get it's changes published. Now as these changes are applied (even the publisher applies 
