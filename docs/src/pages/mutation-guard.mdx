...
## Intent Entries (System Database)

Intent entries provide crash recovery for multi-row transactions. CDC entries captured during preupdate hooks are stored in the system database (`__marmot_system.db`), not custom log files.

### Storage Schema

```sql
CREATE TABLE __marmot__intent_entries (
...
Since the system database is a **separate SQLite file** from user databases, writes to it during preupdate_hook callbacks are safe (no database locking conflicts).

```
┌─────────────────────────────────────────────────────────────┐
│                    EphemeralHookSession                     │
├─────────────────────────────────────────────────────────────┤
│  userDBConn  ──────► user.db (preupdate_hook registered)    │
│  systemDB    ──────► __marmot_system.db (intent writes)     │
│  builders    ──────► In-memory Bloom filter builders        │
└─────────────────────────────────────────────────────────────┘
```

### Performance Benefits

| Operation | File-based (old) | SQLite System DB (current) |
|-----------|------------------|----------------------------|
| Creation | `os.OpenFile` syscall | Nothing (table exists) |
| Per-entry write | Buffered file write | `INSERT` (WAL mode) |
| Durability flush | `file.Sync()` (fsync) | WAL checkpoint (batched) |
| Cleanup | `os.Remove()` (unlink) | `DELETE WHERE txn_id=?` |
| Recovery | Scan dir, parse files | Single DELETE query |

**Single-row operations**: No fsync needed—SQLite WAL mode provides durability automatically.

**Multi-row operations**: `FlushIntentLog()` is now a no-op since SQLite WAL handles durability. Intent entries are cleaned up via DELETE on commit/rollback.
