...
// TestIsVecDropRaceError_VecTable verifies that "no such table" errors on
// CDC-excluded vector tables are recognised as DROP race errors (fix R7).
func TestIsVecDropRaceError_VecTable(t *testing.T) {
	db, cleanup := setupCDCTestDB(t)
	defer cleanup()

	// Attempt to INSERT into a nonexistent vec local table.
	newValues := map[string][]byte{
		"cluster_id": marshalValue(t, int64(1)),
		"rowid":      marshalValue(t, int64(42)),
	}
	err := ApplyCDCInsert(db, "__marmot_vec_emb_members", newValues)
	if err == nil {
		t.Fatal("expected error for nonexistent table")
	}

	if !IsVecDropRaceError("__marmot_vec_emb_members", err) {
		t.Errorf("IsVecDropRaceError should return true for vec local table, got false; err=%v", err)
	}
}

// TestIsVecDropRaceError_NormalTable verifies that "no such table" errors on
// non-vector tables are NOT tolerated.
func TestIsVecDropRaceError_NormalTable(t *testing.T) {
	db, cleanup := setupCDCTestDB(t)
	defer cleanup()

	newValues := map[string][]byte{
		"id":   marshalValue(t, int64(1)),
		"name": marshalValue(t, "test"),
	}
	err := ApplyCDCInsert(db, "normal_table", newValues)
	if err == nil {
		t.Fatal("expected error for nonexistent table")
	}

	if IsVecDropRaceError("normal_table", err) {
		t.Errorf("IsVecDropRaceError should return false for non-vec table, got true; err=%v", err)
	}
}

// TestIsVecDropRaceError_CentroidsTable verifies that replicated centroids
// tables (single underscore) are NOT tolerated — only CDC-excluded tables.
func TestIsVecDropRaceError_CentroidsTable(t *testing.T) {
	db, cleanup := setupCDCTestDB(t)
	defer cleanup()

	newValues := map[string][]byte{
		"index_id": marshalValue(t, int64(1)),
		"version":  marshalValue(t, int64(1)),
	}
	err := ApplyCDCInsert(db, "_marmot_vec_emb_centroids", newValues)
	if err == nil {
		t.Fatal("expected error for nonexistent table")
	}

	if IsVecDropRaceError("_marmot_vec_emb_centroids", err) {
		t.Errorf("IsVecDropRaceError should return false for replicated centroids table, got true")
	}
}

// TestIsVecDropRaceError_NilError verifies nil error returns false.
func TestIsVecDropRaceError_NilError(t *testing.T) {
	if IsVecDropRaceError("__marmot_vec_emb_members", nil) {
		t.Error("IsVecDropRaceError should return false for nil error")
	}
}

// TestIsVecDropRaceError_NonSQLiteError verifies non-SQLite errors return false.
func TestIsVecDropRaceError_NonSQLiteError(t *testing.T) {
	err := fmt.Errorf("some random error")
	if IsVecDropRaceError("__marmot_vec_emb_members", err) {
		t.Error("IsVecDropRaceError should return false for non-SQLite error")
	}
}
